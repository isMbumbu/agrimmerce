...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    ...
    console.log(`Saving file: ${uniqueSuffix + fileExtension}`);
...
app.get('/account', requireLogin, async (req, res) => {
  ...
  console.log('Account access for user:', username);
  try {
  ...
  try {
    ...
    const totalSales = await Order.aggregate([
      { 
        $group: { 
          _id: null, 
          total: { $sum: '$totalPrice' }  // Changed from '$totalAmount' to '$totalPrice' to match your schema
        } 
      }
    ]);
